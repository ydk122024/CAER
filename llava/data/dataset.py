...
from llava.mm_utils import is_gemma_tokenizer, tokenizer_image_token, opencv_extract_frames, process_image_train
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            image_file = self.list_data_dict[i]["image"]
            image_box = self.list_data_dict[i]["bounding_box"]
            #print(type(image_box))
            if isinstance(image_file, list):
                image = torch.stack(
                    [process_image_train(img, box, self.data_args, self.image_folder) for img, box in zip(image_file,image_box)]
                )
            else:
                image = process_image_train(image_file, image_box, self.data_args, self.image_folder)
            sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)
