...
#def process_image(image_file, box, data_args, image_folder):
def process_image(image_file, data_args, image_folder):
    processor = data_args.image_processor
    if isinstance(image_file, str):
        if image_folder is not None:
            image = Image.open(os.path.join(image_folder, image_file)).convert("RGB")
            # image = copy.deepcopy(cv2.imread(os.path.join(image_folder, image_file)))
        else:
            image = Image.open(image_file).convert("RGB")
            #copy.deepcopy(cv2.imread(image_file))
...
#def process_images(images, boxs, image_processor, model_cfg):
def process_images(images, image_processor, model_cfg):
    model_cfg.image_processor = image_processor
    #new_images = [process_image(image, box, model_cfg, None) for image,box in zip(images,boxs)]
...
def process_image_train(image_file, box, data_args, image_folder):
    ...
    if isinstance(image_file, str):
        if image_folder is not None:
            ...
            image =copy.deepcopy(cv2.imread(image_file))
        # box = ast.literal_eval(box)
        x_min, y_min, x_max, y_max = box[0], box[1], box[2], box[3]
        cv2.rectangle(image, (x_min, y_min), (x_max, y_max), (0, 0, 255), 2)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    ...
    if data_args.image_aspect_ratio == "pad":
        ...
        image = processor.preprocess(image, return_tensors="pt")["pixel_values"][0]
    return image
def process_images_train(images, boxs, image_processor, model_cfg):
    model_cfg.image_processor = image_processor
    new_images = [process_image_train(image, box, model_cfg, None) for image,box in zip(images,boxs)]
